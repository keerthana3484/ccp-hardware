...
import type { Medicine } from './reminderWorkflow.js';
...
async function runEndToEndTest() {
  ...
  try {
    ...
    const initialDashData = await initialDashRes.json() as any;
    ...
    const step1StatusData = await step1StatusRes.json() as any;
    ...
    const step1DashData = await step1DashRes.json() as any;
    ...
    const confirmMealData = await confirmMealRes.json() as any;
    ...
    const step2StatusData = await step2StatusRes.json() as any;
    ...
    const step2DashData = await step2DashRes.json() as any;
    ...
    const takeMedData = await takeMedRes.json() as any;
    ...
    const step3StatusData = await step3StatusRes.json() as any;
    ...
    const step3DashData = await step3DashRes.json() as any;
    ...
    events.forEach((ev, idx) => {
      console.log(
        `  [Log #${idx + 1}] Type: ${ev.sensorType.toUpperCase()} | ` +
