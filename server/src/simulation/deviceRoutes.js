...
import { evaluateBatteryAlert, evaluateDeviceOfflineAlert } from './alertService.js';
const router = Router();
/**
...
router.post('/:id/heartbeat', async (req, res) => {
    ...
    try {
        ...
        if (typeof wifiSignalPercent === 'number')
 ...
 */
router.post('/:id/sensor-event', async (req, res) => {
    const deviceId = req.params.id;
 ...
 */
router.post('/:id/compartment/:compartmentId/unlock', async (req, res) => {
    const deviceId = req.params.id;
...
router.post('/simulate', async (req, res) => {
    ...
    try {
        ...
        const nextState = await advanceCompartmentState(deviceId, compNum, event, payload);
        res.status(200).json({
            success: true,
 ...
 */
router.post('/patient/confirm-meal', async (req, res) => {
    ...
    try {
        const compNum = Number(compartmentNumber);
        const nextState = await advanceCompartmentState(deviceId, compNum, 'MEAL_CONFIRMED');
        res.status(200).json({
            success: true,
 ...
 */
router.post('/patient/take-medicine', async (req, res) => {
    const { deviceId, compartmentNumber } = req.body;
    if (!deviceId || compartmentNumber === undefined) {
        res.status(400).json({ error: 'Missing required fields: deviceId, compartmentNumber.' });
        return;
 ...
 */
router.get('/dashboard/medicine-status/:deviceId', async (req, res) => {
