...
import { evaluateBatteryAlert, evaluateDeviceOfflineAlert, evaluateUnscheduledOpenAlert, evaluateCompartmentEmptyAlert } from './alertService.js';
 ...
 */
export async function simulateDeviceTick(deviceId) {
    try {
        ...
        console.log(`[DeviceSimulator][${deviceId}] Telemetry Update: ` +
 ...
 */
export function startDeviceSimulator(deviceId, intervalMs = 15000) {
 ...
 */
export function stopDeviceSimulator(deviceId) {
 ...
 */
export async function advanceCompartmentState(deviceId, compartmentNumber, event, payload) {
    const device = await Device.findOne({ deviceId });
    if (!device) {
    ...
    if (!nextState) {
        throw new Error(`Invalid compartment transition: current state is '${currentState}' but received trigger event '${event}'.`);
    }
    ...
    await device.save();
    console.log(`[DeviceSimulator][${deviceId}][Compartment ${compartmentNumber}] ` +
